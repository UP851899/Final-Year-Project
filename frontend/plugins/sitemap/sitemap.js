...
const openNextPage = $axure.player.openNextPage = function () {
...
const openPreviousPage = $axure.player.openPreviousPage = function () {
  const index = allNodeUrls.indexOf(currentNodeUrl) - 1;
  if (index < 0) return;
  const nextNodeUrl = allNodeUrls[index];
  currentNodeUrl = nextNodeUrl;
...
(function () {
  ...
  const HIGHLIGHT_INTERACTIVE_VAR_NAME = 'hi';

  let currentPageLoc = '';
  let currentPlayerLoc = '';
  let currentPageHashString = '';
  ...
  $(window.document).ready(function () {
    ...
    generateSitemap();
    const pageCount = $('.sitemapPageLink').length;
    ...
    $axure.page.bind('load.sitemap', function () {
      ...
      const $currentNode = $('.sitemapPageLink[nodeUrl="' + currentNodeUrl + '"]');
      ...
      if ($currentNode.length > 0 && pageCount > 1) {
        const currentNode = $currentNode[0];
        const currentNum = $('.sitemapPageLink').index(currentNode) + 1;
      ...
      const hiVal = $axure.player.getHashStringVar(HIGHLIGHT_INTERACTIVE_VAR_NAME);
      ...
      const viewStr = $axure.player.getHashStringVar(ADAPTIVE_VIEW_VAR_NAME);
      if (viewStr.length > 0) {
        ...
        const $viewOption = $('.selectedRadioButton').parents('.adaptiveViewOption');
        $viewOption.click();
      }
      ...
      const scaleStr = $axure.player.getHashStringVar(SCALE_VAR_NAME);
      if (scaleStr.length > 0) {
        const $scale = $('.vpScaleOption[val="' + scaleStr + '"]');
      ...
      const rotateStr = $axure.player.getHashStringVar(ROT_VAR_NAME);
    ...
    const $vpContainer = $('#interfaceScaleListContainer');
  ...
  const _formatViewDimension = function (dim) {
    if (dim == 0) return 'any';
  ...
  function generateAdaptiveViews(forProjectOptions) {
    const $container = forProjectOptions ? $('#projectOptionsAdaptiveViewsContainer') : $('#interfaceAdaptiveViewsListContainer');
    const $viewSelect = forProjectOptions ? $('#projectOptionsViewSelect') : $('#viewSelect');
    const adaptiveViewOptionClass = forProjectOptions ? 'projectOptionsAdaptiveViewRow' : 'adaptiveViewOption';
    const currentViewClass = forProjectOptions ? '' : 'currentAdaptiveView';

    $container.empty();
    $viewSelect.empty();

    // Fill out adaptive view container with prototype's defined adaptive views, as well as the default, and Auto
    let viewsList = '<div class="' + adaptiveViewOptionClass + '" val="auto"><div class="adapViewRadioButton selectedRadioButton"><div class="selectedRadioButtonFill"></div></div>Adaptive</div>';
    let viewSelect = '<option value="auto">Adaptive</option>';
    if (typeof $axure.page.defaultAdaptiveView.name !== 'undefined') {
      ...
      const defaultView = $axure.page.defaultAdaptiveView;
      const defaultViewName = defaultView.name;
    ...
    const useViews = $axure.document.configuration.useViews;
    let hasViews = false;
    if (useViews) {
      for (let viewIndex = 0; viewIndex < $axure.page.adaptiveViews.length; viewIndex++) {
        const currView = $axure.page.adaptiveViews[viewIndex];
  ...
  $axure.messageCenter.addMessageListener(function (message, data) {
    if (message == 'adaptiveViewChange') {
      ...
      if (data.viewId) { $('.adaptiveViewOption[val="' + data.viewId + '"]').addClass('currentAdaptiveView'); } else $('.adaptiveViewOption[val="default"]').addClass('currentAdaptiveView');
  ...
  $axure.player.toggleHotspots = function (val) {
    const overflowMenuCheckbox = $('#showHotspotsOption').find('.overflowOptionCheckbox');
  ...
  function showHotspots_click(event) {
    const overflowMenuCheckbox = $('#showHotspotsOption').find('.overflowOptionCheckbox');
    const projOptionsCheckbox = $('#projectOptionsHotspotsCheckbox');
  ...
  function adaptiveViewOption_click(event) {
    ...
    $('.adaptiveViewOption').removeClass('currentAdaptiveView');
    if (currVal) { $('.adaptiveViewOption[val="' + currVal + '"]').addClass('currentAdaptiveView'); } else $('.adaptiveViewOption[val="default"]').addClass('currentAdaptiveView');
  ...
  var selectAdaptiveView = $axure.player.selectAdaptiveView = function (currVal) {
    if (currVal == 'auto') {
      ...
      currentPageLoc = $axure.page.location.split('#')[0];
      const decodedPageLoc = decodeURI(currentPageLoc);
      const nodeUrl = decodedPageLoc.substr(decodedPageLoc.lastIndexOf('/')
  ...
  $axure.player.updateAdaptiveViewHeader = updateAdaptiveViewHeader = function () {
    let hasDefinedDim = true;
    const dimensionlessViewStr = '(any x any)';

    let viewString = $('.adaptiveViewOption.currentAdaptiveView').text();
    if (viewString != null && viewString.indexOf(dimensionlessViewStr) >= 0) hasDefinedDim = false;

    if (!hasDefinedDim) {
      const viewName = viewString.substring(0, viewString.lastIndexOf(' ('));
      const widthString = $('#mainPanelContainer').width();
  ...
  $axure.player.selectScaleOption = function (scaleVal) {
    const $scale = $('.vpScaleOption[val="' + scaleVal + '"]');
    if ($scale.length > 0) $scale.click();
  };

  function vpScaleOption_click(event) {
    const scaleCheckDiv = $(this).find('.scaleRadioButton');
    const scaleVal = $(this).attr('val');
    if (scaleVal == '0') {
      $axure.player.zoomPage(100);
    }
    if (scaleCheckDiv.hasClass('selectedRadioButton')) return false;

    const $selectedScaleOption = $('.vpScaleOption[val="' + scaleVal + '"], .projectOptionsScaleRow[val="' + scaleVal + '"]');
    const $allScaleOptions = $('.vpScaleOption, .projectOptionsScaleRow');
  ...
  function generateSitemap() {
    let treeUl = "<div id='sitemapHeader'' class='sitemapHeader'>";
    ...
    let sitemapTitle = $axure.player.getProjectName();
